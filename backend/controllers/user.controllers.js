...
const users=await User.find({
  _id:{$ne:req.userId}}).select("-password")
...
//       targetUser.followers=targetUser.followers.filter(id=>id.
//       toString()!=currentUserId)
//       await currentUser.save();
//       await targetUser.save();
//       return res.status(200).json({following:true,message:"unfollow successfully"})
...
//        await currentUser.save();
//       await targetUser.save();
//       return res.status(200).json({following:false,message:" follow successfully"})

//     }

//   }catch(error){
//       return res.status(500).json({message:`you cannot follow yourself${error}`})
//     }
//   }
export const follow = async (req, res) => {
  try {
